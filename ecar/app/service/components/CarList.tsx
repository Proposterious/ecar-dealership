...
function CarList() { 
    ...
    async function formCars() {
        ...
        console.log('Should be the cars as an array', array); // log prior to 'cars' mutations
    ...
    return (
    <>
        <section className="flex flex-row | first:invisible hover:first:visible | fixed right-8 top-36">
            <div id="carList-tooltip" className="bg-slate-200/90 px-3 py-2 m-3 | font-semibold text-sm text-slate-600">
            ...
            <span className="max-w-fit my-auto">
                ...
                onMouseEnter={() => {
                    document.getElementById('carList-tooltip')!.classList.remove('invisible');
                }}
                onMouseLeave={() => {
                document.getElementById('carList-tooltip')!.classList.add('invisible');
                }}>
                        ?
                </button>
            </span>
        ...
        <section className="bg-inherit | grid grid-cols-4 grid-flow-row">
          {data && data.map((dict: any) => (
            ...
            {/* display expanded form of carList */}
              {format == "expand" && dict.map((car: any) => (
                <ul key={car.id} className="bg-slate-100 font-semibold text-center text-lg space-y-1 shadow-xs transition duration-300 ease-out hover:shadow-lg shadow-orange-700/70 rounded-lg hover:cursor-default">
